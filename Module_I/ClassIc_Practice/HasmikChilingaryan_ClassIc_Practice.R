...
#             Module I: R Basic Syntax (Class Ic)
...
cholesterol <- 230 #assigned cholesterol value
...
Systolic_bp <- 130 #assigned systolic blood pressure value

if (Systolic_bp < 120) {
  print("Blood Pressure is normal")
} else {
  print("Blood Prressure is high")
}
...
#convert patint_info columns

clean_patient <- patient_data
factor_cols_patient <- c("gender", "diagnosis", "smoker")

for (col in factor_cols_patient) {
  clean_patient[[col]] <- as.factor(clean_patient[[col]])
}

#convert metadata columns

clean_metadata <- metadata
factor_cols_meta <- c("name", "height", "gender")
...
#verify results
cat("Structure of Clean Patient Data:\n")
str(clean_patient)

cat("\nStructure of Clean Metadata:\n")
str(clean_metadata)
...
#copy for binary conversion
binary_data <- clean_data

binary_cols <- c("smoker")

for (col in binary_cols) {
  binary_data[[col]] <- ifelse(binary_data[[col]] == "Yes", 1, 0)
}
...
save.image("HasmikChilingaryan_ClassIc_Practice.RData")
